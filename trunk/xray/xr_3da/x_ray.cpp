...
#ifdef CLEANING_CONFIGS
	FS.update_path				(fname,"$game_config$","game\\game.ltx");
#else
...
void CApplication::LoadTitleLs(LPCSTR str)
{
	strcpy_s					(ls_title, str);
}
...
//progress bar
		...
		back_text_coords.lt.set		(4,772);back_text_coords.rb.add(back_text_coords.lt,back_size);
		back_coords.lt.set			(273 ,582);back_coords.rb.add(back_coords.lt,back_size);
