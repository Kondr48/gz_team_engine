...
void CWeaponKnife::Load	(LPCSTR section)
{
	...
	animGet             (mhud_idle_moving, pSettings->r_string(*hud_sect,"anim_idle_moving") );
...
bool CWeaponKnife::TryPlayAnimIdle()
{
	VERIFY(GetState() == eIdle);
		...
		if (pActor)
		{
			...
			else if (st.bMoving && mhud_idle_moving.size())
			{
				m_pHUD->animPlay(random_anim(mhud_idle_moving), TRUE, NULL, GetState());
				return true;
			}
		}
	return false;
