...
void CWeaponShotgun::Load	(LPCSTR section)
{
	...
	if(pSettings->line_exist(section, "duplet_status")) //Kondr48: для выстрела дуплетом на кнопку ПГ
		m_bDupletStatus = !!pSettings->r_bool(section, "duplet_status");
...
bool CWeaponShotgun::Action			(s32 cmd, u32 flags) 
{
	...
	switch(cmd) 
	{
		case kWPN_FUNC : //Kondr48: выстрел дуплетом на кнопку ПГ
			{
				if (m_bDupletStatus && m_bDupletStatus == true)
				{
				if(flags&CMD_START) 
				 Fire2Start();
				else 
...
void CWeaponShotgun::OnAnimationEnd(u32 state) 
{
	...
	switch(m_sub_state){
		case eSubstateReloadBegin:{		
			if(iAmmoElapsed > 0) //anim_rouge: зарядка первого патрона в оружие
			{
				if (m_bBoltAction == true)
					--iAmmoElapsed; //anim_rouge: уменьшим количество патронов (выбросим из ствола)
				else 
					m_sub_state = eSubstateReloadInProcess;
			}
			else if (0 != AddCartridge(1)) //anim_rouge: наркоманский вызов зарядки одного патрона. наследие GSC
			 m_sub_state = eSubstateReloadInProcess;
