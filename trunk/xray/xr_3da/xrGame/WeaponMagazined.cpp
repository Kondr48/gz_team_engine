...
CWeaponMagazined::CWeaponMagazined(LPCSTR name, ESoundTypes eSoundType) : CWeapon(name)
{
	...
	m_eSoundReloadEmpty = ESoundTypes(SOUND_TYPE_WEAPON_RECHARGING | eSoundType); //Kondr48: reload_empty
	m_eSoundReloadMisfire = ESoundTypes(SOUND_TYPE_WEAPON_RECHARGING | eSoundType); //anim_rouge, Kondr48: расклин.	
...
CWeaponMagazined::~CWeaponMagazined()
{
	...
	HUD_SOUND::DestroySound(sndReloadEmpty);    //Kondr48: reload_empty
	HUD_SOUND::DestroySound(sndReloadMisfire);  //anim_rouge, Kondr48: расклин.	
}
...
void CWeaponMagazined::StopHUDSounds()
{
	...
	HUD_SOUND::StopSound(sndReloadEmpty);       //Kondr48: reload_empty
	HUD_SOUND::StopSound(sndReloadMisfire);     //anim_rouge, Kondr48: расклин.	
...
void CWeaponMagazined::Load(LPCSTR section)
{
	...
	HUD_SOUND::LoadSound(section, "snd_reload_missfire", sndReloadMisfire, m_eSoundReloadMisfire);   //anim_rouge, Kondr48: расклин.
	if (pSettings->line_exist(section, "snd_reload_empty")) //Kondr48: reload_empty.
...
void CWeaponMagazined::FireStart()
{
	if (IsValid() && !IsMisfire())
	{
		if (!IsWorking() || AllowFireWhileWorking())
		{
			...
			if (GetState() == eReloadMisfire) return;
...
void CWeaponMagazined::Chamber()  //Kondr48: патрон в патроннике
{
...
void CWeaponMagazined::ReloadMisfire() //anim_rouge, Kondr48: расклин.	
{
	if (IsMisfire())	bMisfire = false;
...
void CWeaponMagazined::UpdateSounds()
{
	...
	if (sndReloadEmpty.playing()) sndReloadEmpty.set_position(get_LastFP());     //Kondr48: reload_empty
	if (sndReloadMisfire.playing()) sndReloadMisfire.set_position(get_LastFP()); //anim_rouge, Kondr48: расклин.	         
}
...
void CWeaponMagazined::OnAnimationEnd(u32 state)
{
	switch (state)
	{
	case eReload:	 Chamber(); ReloadMagazine(); SwitchState(eIdle);	break;	// //Kondr48: патрон в патроннике
	case eReloadMisfire:	ReloadMisfire();	SwitchState(eIdle);	    break;	//anim_rouge, Kondr48: расклин.	
...
{
	if (GetAmmoElapsed() == 0) //Kondr48: reload_empty
...
void CWeaponMagazined::switch2_ReloadMisfire()
{
	PlaySound(sndReloadMisfire, get_LastFP());
	PlayAnimReloadMisfire();
	m_bPending = true;
}
