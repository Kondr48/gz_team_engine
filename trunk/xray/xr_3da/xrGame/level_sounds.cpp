...
#endif
	string_path					_fn;  
	if (FS.exist(_fn,"$game_sounds$",fn,".ogg"))
		m_SourceLeft.create(fn,st_Music,sg_Undefined);
	else{
		string_path			_l, _r;
...
{
	if (!m_SourceRight._p)
		m_SourceLeft.play_at_pos	(0,Fvector().set(0.0f,0.0f,0.0f),sm_2D);
	else{
...
{
	BOOL  ret = (NULL!=m_SourceLeft._feedback());
	return ret;
}
