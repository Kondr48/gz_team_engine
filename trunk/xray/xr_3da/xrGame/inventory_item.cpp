...
//.	NameComplex			();
	...
	m_can_be_hacked			    = READ_IF_EXISTS(pSettings, r_bool, section, "can_be_hacked",	    FALSE);
	m_quest_device			    = READ_IF_EXISTS(pSettings, r_bool, section, "quest_device",	    FALSE);

	if(m_can_be_hacked == true)
		m_encryption_level = pSettings->r_string(section, "encryption_level");

	if ( pSettings->line_exist(section, "soft_level") )
		m_soft_level = pSettings->r_s32(section, "soft_level");
