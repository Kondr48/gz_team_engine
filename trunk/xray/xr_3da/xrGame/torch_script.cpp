...
void CTorch::script_register	(lua_State *L)
{
	...
	[
		class_<CTorch,CInventoryItemObject>("CTorch")
			...
			.def("get_battarey_power" ,		&CTorch::GetBattareyPower)	
			.def("set_battarey_power" ,		&CTorch::SetBattareyPower)
			.def("get_battarey_life"  ,		&CTorch::GetBattareyLife)
			.def("set_battarey_life"  ,		&CTorch::SetBattareyLife)
			,			
