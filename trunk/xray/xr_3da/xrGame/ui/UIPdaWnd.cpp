...
void CUIPdaWnd::SetActiveSubdialog(EPdaTabs section)
{
	...
	case eptQuests:
		...
		break;
	case eptHack:
		m_pActiveDialog			= smart_cast<CUIWindow*>(UIHackWnd);
		InventoryUtilities::SendInfoToActor("ui_pda_hack");
		UIHackWnd->ClearRightWindow();
		g_pda_info_state		&= ~pda_section::hack;
		break;
	default:
...
void CUIPdaWnd::DrawUpdatedSections				()
{
	...
	pos = m_sign_places_main[eptHack];
	pos.add(tab_pos);
	if(g_pda_info_state&pda_section::hack)
		draw_sign								(m_updatedSectionImage, pos);
	else
		draw_sign								(m_oldSectionImage, pos);
