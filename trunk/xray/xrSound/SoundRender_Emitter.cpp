...
{ 
	if(source()->channels_num()==1){
		...
		if (p_source.position_set)
		{
			static const float alpha = 0.05f;
			p_source.prevVelocity.set(p_source.accVelocity);
			p_source.curVelocity.sub(pos, p_source.position);
			p_source.accVelocity.set(p_source.curVelocity.mul(alpha).add(p_source.prevVelocity.mul(1.f-alpha)));
		}
		else
		{
			p_source.position_set = true;
		}
