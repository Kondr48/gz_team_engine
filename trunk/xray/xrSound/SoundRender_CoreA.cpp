...
void CSoundRender_CoreA::update_listener		( const Fvector& P, const Fvector& D, const Fvector& N, float dt )
{
	...
	if (dt > 0) {
		Fvector preVel; preVel.set(Listener.accVelocity);
		Fvector curVel; curVel.sub(P, Listener.position);
//		Msg("curVel(%f)[%f:%f:%f] preVel(%f)[%f:%f:%f] dt %f",curVel.magnitude(),curVel.x,curVel.y,curVel.z, preVel.magnitude(),preVel.x,preVel.y,preVel.z,dt);
		if (!preVel.similar(curVel,0.01f) && curVel.magnitude() < 10) {	// отсекаем спавн и телепорт
			static const float alpha = 0.1f;
			Listener.accVelocity.set(curVel.mul(alpha).add(preVel.mul(1.f - alpha)));  
			Listener.velocity.set(Listener.accVelocity).div(dt);
		}
//		else
//			Msg("curVel.magnitude() > 10");
	}
	else
		Msg("update_listener:dt<=0 %f",dt);
