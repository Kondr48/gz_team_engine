...
void CSoundRender_Emitter::update(float dt)
{
	...
	case stStarting:
		if (iPaused)						break;
		fTimeStarted						= fTime;
		fTimeToStop							= fTime + get_length_sec();	// doppler!
...
void CSoundRender_Emitter::update_environment(float dt)
{
	if (bMoved){
		...
		// Cribbledirge: update the velocity of the sound.
		if (dt > 0 && p_source.position_set)
			p_source.velocity.set(p_source.accVelocity).div(dt);
	}
