...
half4 	plight_infinity		(half m, half3 point, half3 normal, half3 light_direction)       				{
  ...
  half4 ret;
  half4 light 	= tex3D 	(s_material, 	half3( dot(L,N), dot(H,N), m ) ).xxxy;
  ret 	= light * half4(light.www * (Ldynamic_color.xyz * 4), light.w) * Ldynamic_color.w/4;
...
half4 	plight_local		(half m, half3 point, half3 normal, half3 light_position, half light_range_rsq, out float rsqr)  {
  ...
  half4 light 	= tex3D 	(s_material, half3( dot(L,N), dot(H,N), m ) ).xxxy; 
  return att*light + att*half4(light.www * (Ldynamic_color.xyz*4), light.w) * Ldynamic_color.w/4;
  //return att*light;
}
